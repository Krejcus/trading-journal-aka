...
1. **TypeScript:** `npx tsc --noEmit`. Porovnej proti `.Codex/tsc-baseline.txt` — projdou jen pokud nejsou NOVÉ chyby (pre-existující baseline chyby jsou OK). Pokud přibyla chyba, oprav ji a nepokračuj.

2. **Build sanity (volitelně u větších změn):** `npm run build` musí projít (Vite). U malých UI změn stačí tsc + preview.

3. **Preview ověření:** pokud je změna pozorovatelná v browseru, ověř přes Codex Preview (console bez nových errorů, screenshot klíčové obrazovky). Nikdy nenech uživatele ověřovat ručně.
