...
export class ChartWorkspaceHistory {
  ...
  updatePanels = (panels: Map<string, WorkspaceHistoryPanel>) => {
    ...
    this.panels.forEach((panel, id) => {
      ...
      if (panel.chartApi) {
        ...
        const listener = () => this.capture('posun nebo časové měřítko grafu');
  ...
  capture = (label: string) => {
    ...
    const next = this.snapshot();
    if (sameSnapshot(next, this.pending?.snapshot ?? this.current)) return;
  ...
  captureAfterEvent = (label: string) => {
    ...
    window.requestAnimationFrame(() => this.capture(label));
  };
