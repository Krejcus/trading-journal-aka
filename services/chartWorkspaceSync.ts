...
/** Časový rozsah, který má panel právě na obrazovce (v sekundách). */
const visibleTimeRange = (chart: IChartApi): { from: number; to: number } | null => {
  try {
    ...
    // Rozebraný graf při přeskládání layoutu — synchronizace ho jen přeskočí.
    return null;
  }
};

/**
 * Svíčka, na kterou se má přenést kurzor z druhého panelu — nebo `null`, když
 * tam pro daný čas není co ukázat.
 *
 * Nejbližší svíčka sama o sobě nestačí: hledání se na okrajích zarazí o krajní
 * prvek pole, takže čas mimo data skončil vždycky na první nebo poslední
 * svíčce. Panel odscrollovaný do jiného období pak dostal kurzor přilepený na
 * kraj grafu — a protože ho tam vracela každá další událost, sám nezmizel.
 */
export const syncedCrosshairCandle = (params: {
  candles: MarketCandle[];
  time: number;
  visibleRange: { from: number; to: number } | null;
}): MarketCandle | null => {
  const { candles, time, visibleRange } = params;
  if (!Number.isFinite(time)) return null;
  const candle = nearestCandle(candles, time);
  if (!candle || !visibleRange) return null;
  // Porovnáváme čas kurzoru, ne nalezené svíčky: na vyšším timeframe leží
  // začátek svíčky klidně hodiny před pozicí kurzoru, ale ukázat ji chceme.
  const withinView = time >= visibleRange.from && time <= visibleRange.to;
  const withinData = time >= candles[0].time && time <= candles[candles.length - 1].time;
  return withinView && withinData ? candle : null;
};
    ...
    if (settings.crosshair) {
      const handleCrosshair: Parameters<IChartApi['subscribeCrosshairMove']>[0] = param => {
        ...
        controls.forEach(([targetId, target]) => {
          ...
          const candle = syncedCrosshairCandle({
            candles: currentCandles(target),
            time: Number(param.time),
            visibleRange: visibleTimeRange(targetChart),
          });
          // Panel odscrollovaný jinam nemá co ukazovat. Bez tohohle se mu
          // kurzor přilepil na kraj grafu a zůstal tam, dokud uživatel nehýbal
          // myší přímo nad ním.
          if (!candle) {
            targetChart.clearCrosshairPosition();
            return;
