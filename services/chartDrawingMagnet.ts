...
interface DrawingEngineLike {
  ...
  getById: (id: string) => { points: DrawingPoint[]; tool?: string } | undefined;
  setPoints: (id: string, points: DrawingPoint[], pointerPoint?: DrawingPoint) => void;
}
...
export const createDrawingMagnetResolver = (
  sources: DrawingMagnetSources | (() => DrawingMagnetSources),
  projector: DrawingMagnetProjector,
) => (point: DrawingPoint): DrawingPoint => {
  // Replay appends candles without remounting the chart. Resolve the source at
  // pointer time so the magnet never remains capped at an older replay cursor.
  const currentSources = typeof sources === 'function' ? sources() : sources;
  const settings = getDrawingMagnetSettings();
  if (settings.mode === 'off' || currentSources.candles.length === 0) return point;
  ...
  const candleIndex = nearestIndex(currentSources.candles, point.time);
  const nearestCandle = currentSources.candles[candleIndex];
  const candidates: DrawingPoint[] = [];
  for (let index = Math.max(0, candleIndex - 1); index <= Math.min(currentSources.candles.length - 1, candleIndex + 1); index += 1) {
    const candle = currentSources.candles[index];
...
export const installDrawingMagnet = (
  ...
  const setPoints = (id: string, points: DrawingPoint[], pointerPoint?: DrawingPoint) => {
    if (id.startsWith('auto-')) {
      original.setPoints.call(engine, id, points);
      return;
    }
    const drawing = engine.getById(id);
    const previous = drawing?.points ?? [];
    ...
    const isPosition = drawing?.tool === 'LongPosition' || drawing?.tool === 'ShortPosition';
    const changedPositionPriceIndices = isPosition
      ? points.flatMap((point, index) => previous[index]?.price !== point.price ? [index] : [])
      : [];
    const reshapedPositionIndex = pointerPoint && changedPositionPriceIndices.length === 1
      ? changedPositionPriceIndices[0]
      : null;
    const shouldSnap = changedCount === 1 || reshapedPositionIndex !== null;
    const resolved = shouldSnap ? points.map((point, index) => {
      if (reshapedPositionIndex !== null && index !== reshapedPositionIndex) return point;
      if (!changed[index]) return point;
      // Position target/stop anchors store the right edge time even though their
      // visible handle is on the left. Snap against the candle under the pointer,
      // then keep the stored time so TP/SL adjustment never changes box width.
      if (isPosition && (index === 1 || index === 2) && points[0]) {
        const snapped = resolver({ time: pointerPoint?.time ?? points[0].time, price: point.price });
        return { ...point, price: snapped.price };
      }
      return resolver(point);
    }) : points;
