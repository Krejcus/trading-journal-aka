...
| Ten Home/Lock Screen widgets | Partially physically verified | Seven Home Screen variants plus dedicated Lock Screen P&L, Discipline, and LIVE widgets cover inline, circular, and rectangular families; the user physically confirmed all three Lock Screen widgets on the target iPhone, including opening the app from LIVE, plus Home Screen Equity Curve, Daily P&L, Accounts, Today Plan, and Discipline with their labelled test data. The Lock Screen Discipline pass also confirmed the required `containerBackground` fix in the signed build | Complete Recent Trades and Quick Actions Home Screen smokes; live values require paid Apple Developer App Group and a token-free snapshot writer |
