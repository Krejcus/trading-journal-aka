...
const reviewedTrade: Trade = {
  ...
  entryContext: {
    placement: {
      slPlacement: 'fvg',
      slCandidates: {
        ote: { price: 21994, distanceTicks: 4, matched: false },
        swing: { price: 21992, distanceTicks: 12, matched: false },
        fvg: { price: 21995, distanceTicks: 0, matched: true },
      },
      targetPolicy: {
...
describe('BacktestTradeReviewDialog render', () => {
  it('shows actual collected values and the existing saved snapshot', () => {
    ...
    expect(markup).toContain('Nejbližší PDH');
    expect(markup).toContain('Chráněný extrém 21990.00');
  ...
  it('neprezentuje vzdálený level jako potvrzený cíl', () => {
    ...
    expect(markup).toContain('Nejbližší kandidát WO');
