...
describe('local Tradovate read-only proxy', () => {
  it('allows only account observation endpoints', () => {
    ...
    expect(isAllowedLocalTradovateRead('GET', '/api/tradovate/oauth/copier-relay')).toBe(true);
  ...
  it('blocks OAuth, disconnects, profile writes, and unrelated APIs', () => {
    ...
    expect(isAllowedLocalTradovateRead('POST', '/api/tradovate/oauth/copier-relay')).toBe(false);
    expect(isAllowedLocalTradovateRead('DELETE', '/api/tradovate/oauth/copier-relay')).toBe(false);
