...
describe('durable notification delivery', () => {
  ...
  it('keeps the 45s image deadline after slow preparation and queued retry while preserving one text alert', async () => {
    const { db, api, queue } = database(); await enqueue(db);
    queue[0].payload.imageStoragePath = 'u/chart.png'; queue[0].payload.imageDeadlineAt = NOW + 45_000;
    api.storage.from = () => ({ createSignedUrl: async () => {
      vi.mocked(Date.now).mockReturnValue(NOW + 46_000);
      return { data: { signedUrl: 'https://example.test/too-late.png' }, error: null };
    } });
    expect((await drainNotificationDeliveries({ db })).sent).toBe(1);
    expect(sendApnsNotification).toHaveBeenCalledTimes(1);
    expect(vi.mocked(sendApnsNotification).mock.calls[0][1]).not.toHaveProperty('imageUrl');
    expect(vi.mocked(sendApnsNotification).mock.calls[0][1]).not.toHaveProperty('mutableContent');
    expect(queue[0].status).toBe('sent');
    const second = database(); await enqueue(second.db); second.queue[0].payload.imageStoragePath = 'u/retry.png';
    second.queue[0].payload.imageDeadlineAt = NOW + 45_000;
    const sign = vi.fn(); second.api.storage.from = () => ({ createSignedUrl: sign });
    await drainNotificationDeliveries({ db: second.db });
    expect(sign).not.toHaveBeenCalled();
    expect(vi.mocked(sendApnsNotification).mock.calls[1][1]).not.toHaveProperty('imageUrl');
  });

  it('does not enqueue an image after its worker deadline has already expired', async () => {
    const { db, queue } = database();
    expect(await sendCopierSnapshotFollowUp({ db, userId: USER, deviceId: DEVICE, storagePath: 'u/chart.png',
      input: { ...event, notifyDeadlineAt: NOW - 1 } as any })).toBeNull();
    expect(queue).toEqual([]); expect(sendApnsNotification).not.toHaveBeenCalled();
  });
