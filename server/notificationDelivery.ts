...
export type NotificationPayload = ApnsNotification & { imageStoragePath?: string };
  ...
  for (let offset = 0; offset < rows.length; offset += 10) {
    await Promise.all(rows.slice(offset, offset + 10).map(async row => {
      ...
      try {
        ...
        const { imageStoragePath, ...payload } = row.payload;
        if (row.channel === 'apns' && imageStoragePath) {
