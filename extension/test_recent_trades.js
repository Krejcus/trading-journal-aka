...
async function check() {
  // Check for trades from last hour
  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000).toISOString();
  const { data, error } = await supabase.from('trades')
    .select('id, user_id, account_id, signal, date, created_at, instrument, pnl')
    .gte('created_at', oneHourAgo)
    .order('created_at', { ascending: false });
  
  console.log("TRADES FROM LAST HOUR:");
  console.log(error ? JSON.stringify(error) : (data?.length ? JSON.stringify(data, null, 2) : "NONE FOUND"));
  
  // Total count
  const { count } = await supabase.from('trades').select('*', { count: 'exact', head: true });
  console.log(`\nTOTAL TRADES IN DB: ${count}`);
}
check();
