...
async function check() {
  const { data, error } = await supabase.rpc('get_policies_for_trades'); // If exists, or just try to insert one with a service role. Wait, we don't have service role key.
}
check();
