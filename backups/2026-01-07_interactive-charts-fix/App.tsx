...
import { Trade, Account, TradeFilters, CustomEmotion, User, DailyPrep, DailyReview, UserPreferences, DashboardWidgetConfig, SessionConfig, IronRule, BusinessExpense, BusinessPayout, PlaybookItem, BusinessGoal, BusinessResource, BusinessSettings, PsychoMetricConfig } from './types';
...
const App: React.FC = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (prefs) {
          ...
          if (prefs.businessSettings) setBusinessSettings(prefs.businessSettings);
  ...
  }, [userEmotions, userMistakes, standardGoals, dashboardLayout, sessions, htfOptions, ltfOptions, ironRules, instrumentFees, businessExpenses, businessPayouts, playbookItems, businessGoals, businessResources, businessSettings, psychoMetrics, theme, sharedTrade, session, isInitialLoadDone]);
  ...
  const displayBalance = useMemo(() => {
    ...
    // Sum initial balances of all non-copy accounts (masters or independent)
    return accounts
      .filter(a => !a.parentAccountId)
      .reduce((sum, a) => sum + (a.initialBalance || 0), 0);
  }, [accounts, activeAccount, viewMode]);
  ...
  const handleManualTrade = (tradeOrTrades: Trade | Trade[]) => {
    const newTrades = Array.isArray(tradeOrTrades) ? tradeOrTrades : [tradeOrTrades];
  ...
  return (
    <div className={`min-h-screen font-sans flex transition-colors duration-300 ${theme === 'light' ? 'light-theme' : (theme === 'oled' ? 'oled-theme' : '')} bg-[var(--bg-page)] text-[var(--text-primary)]`}>
      <Sidebar
      ...
      <main className={`flex-1 transition-all duration-300 relative flex flex-col ${isSidebarCollapsed ? 'lg:ml-24' : 'lg:ml-72'}`}>
        <header className={`sticky top-0 z-40 border-b backdrop-blur-md px-6 py-4 flex items-center justify-between transition-all bg-[var(--bg-page)]/80 border-[var(--border-subtle)]`}>
          ...
          <div className="flex items-center gap-6">
            <div className={`hidden md:flex p-1 rounded-xl border transition-all ${theme !== 'light' ? 'bg-[var(--bg-input)]/30 border-[var(--border-subtle)]' : 'bg-slate-100 border-slate-200 shadow-inner'}`}>
              <button
                onClick={() => setViewMode('individual')}
                className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase flex items-center gap-2 transition-all ${viewMode === 'individual' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-500 hover:text-slate-400'}`}
              >
                <UserIcon size={12} /> Singular
              </button>
              <button
                onClick={() => setViewMode('combined')}
                className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase flex items-center gap-2 transition-all ${viewMode === 'combined' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-500 hover:text-slate-400'}`}
              >
                <Users size={12} /> Combined
              </button>
            </div>
            ...
            <>
              ...
              {activePage === 'settings' && (
                <Settings
                  ...
                  psychoMetrics={psychoMetrics} setPsychoMetrics={setPsychoMetrics}
                />
              )}
