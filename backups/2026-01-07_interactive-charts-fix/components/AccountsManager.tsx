...
const AccountsManager: React.FC<AccountsManagerProps> = ({ accounts, activeAccountId, setActiveAccountId, onUpdate, onDelete, theme, trades }) => {
  ...
  const sortedAccounts = useMemo(() => {
    const list = showInactive ? inactiveAccounts : activeAccounts;
    const masters = list.filter(a => !a.parentAccountId);
    const sorted: Account[] = [];

    masters.forEach(m => {
      sorted.push(m);
      const copies = list.filter(a => a.parentAccountId === m.id);
      sorted.push(...copies);
    });

    // Add orphaned copies or others just in case
    list.forEach(a => {
      if (!sorted.find(s => s.id === a.id)) {
        sorted.push(a);
      }
    });

    return sorted;
  }, [showInactive, activeAccounts, inactiveAccounts]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <section className={`p-8 rounded-[40px] border bg-gradient-to-br from-blue-600/5 to-indigo-600/5 ${theme !== 'light' ? 'border-[var(--border-subtle)]' : 'border-slate-200 shadow-xl'}`}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            <div className="text-right">
              ...
              <p className="text-xl font-black font-mono text-rose-500">-${portfolioStats.totalChallengeCosts.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
            </div>
            <div className="text-right">
              ...
              <p className={`text-xl font-black font-mono text-slate-400`}>
                ${portfolioStats.totalTradesPnL.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </p>
            </div>
            <div className="text-right">
            ...
            <div className="text-right">
              ...
              <p className="text-xl font-black font-mono text-emerald-500">
                +${portfolioStats.totalNetPayouts.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </p>
            </div>
            <div className={`text-right border-t lg:border-t-0 lg:border-l pt-4 lg:pt-0 lg:pl-8 ${theme !== 'light' ? 'border-[var(--border-subtle)]' : 'border-slate-100'}`}>
              ...
              <p className={`text-3xl font-black font-mono tracking-tighter ${portfolioStats.netCashflow >= 0 ? 'text-blue-500' : 'text-rose-600'}`}>
                {portfolioStats.netCashflow >= 0 ? '+' : ''}${portfolioStats.netCashflow.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </p>
            </div>
          </div>
        </div>
      ...
      {isAdding && (
        <div className={`p-10 rounded-[40px] border-2 border-dashed animate-in slide-in-from-top-4 duration-500 ${theme !== 'light' ? 'bg-[var(--bg-card)]/50 border-[var(--border-subtle)]' : 'bg-white border-slate-200 shadow-2xl'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
            ...
            <div className="space-y-2">
              <label className="text-[9px] uppercase font-black text-slate-500 tracking-widest text-emerald-500">Prop Threshold ($/day)</label>
              <input type="number" value={newAccount.propThreshold} onChange={e => setNewAccount({ ...newAccount, propThreshold: Number(e.target.value) })} className={`${inputClass} border-emerald-500/20`} placeholder="150" />
            </div>
            <div className="space-y-2">
              <label className="text-[9px] uppercase font-black text-slate-500 tracking-widest text-blue-500">Master Účet (Kopírovat)</label>
              <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sortedAccounts.map(acc => {
          ...
          return (
            <div
              key={acc.id}
              onClick={() => !isEditing && acc.status === 'Active' && setActiveAccountId(acc.id)}
              className={`group relative p-8 rounded-[40px] border-2 transition-all duration-500 cursor-pointer overflow-hidden 
                ${activeAccountId === acc.id && !isEditing ? 'border-blue-600 bg-blue-600/5 shadow-2xl' : (theme !== 'light' ? 'bg-[var(--bg-card)] border-[var(--border-subtle)]' : 'bg-white border-slate-100')} 
                ${acc.status === 'Inactive' ? 'opacity-70 grayscale-[0.5]' : ''}
                ${acc.parentAccountId ? 'ml-0 md:ml-8 scale-[0.98] border-dashed border-blue-500/30' : ''}`}
            >

              {/* Header Actions */}
              <div className="flex justify-between items-start mb-8 relative z-20">
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded-[24px] ${theme !== 'light' ? 'bg-[var(--bg-page)] shadow-inner' : 'bg-slate-50'}`}>{getAccountIcon(acc.type, acc.status, acc.phase)}</div>
                  {isEditing ? (
                    <input value={editFormData.name} onClick={e => e.stopPropagation()} onChange={e => setEditFormData({ ...editFormData, name: e.target.value })} className={`px-3 py-1.5 text-xs font-black uppercase tracking-widest rounded-lg outline-none ${theme !== 'light' ? 'bg-[var(--bg-input)] text-white border border-[var(--border-subtle)] focus:border-blue-500' : 'bg-slate-100 border border-slate-200'}`} />
                  ) : (
                    <div>
                      <h3 className={`font-black uppercase tracking-tight ${theme !== 'light' ? 'text-white' : 'text-slate-900'}`}>{acc.name}</h3>
                      <div className="flex items-center gap-2">
                        ...
                        {acc.parentAccountId && (
                          <>
                            <span className="text-slate-700 text-[8px]">•</span>
                            <span className="text-[9px] uppercase font-black text-blue-500 tracking-tighter flex items-center gap-1">
                              <Zap size={8} /> Copying {accounts.find(a => a.id === acc.parentAccountId)?.name || 'Master'}
                            </span>
                          </>
                        )}
                      </div>
              ...
              <div className="space-y-6 relative z-20">
                ...
                {isEditing && (
                  <div className={`mt-4 p-4 rounded-2xl border ${theme !== 'light' ? 'bg-[var(--bg-page)]/40 border-[var(--border-subtle)]' : 'bg-slate-50 border-slate-100 shadow-inner'}`} onClick={e => e.stopPropagation()}>
                    <label className="text-[9px] uppercase font-black text-slate-500 tracking-widest block mb-1.5">Master Účet (Kopírovat)</label>
                    <select
                      value={editFormData.parentAccountId || ''}
                      onChange={e => setEditFormData({ ...editFormData, parentAccountId: e.target.value || undefined })}
                      className={`w-full px-4 py-2 rounded-xl border text-xs font-bold outline-none ring-blue-500/30 focus:ring-2 ${theme !== 'light' ? 'bg-[var(--bg-input)] border-[var(--border-subtle)] text-white' : 'bg-white border-slate-200'}`}
                    >
                      <option value="">-- Samostatný účet --</option>
                      {accounts.filter(a => a.id !== acc.id && !a.parentAccountId).map(a => (
                        <option key={a.id} value={a.id}>{a.name}</option>
                      ))}
                    </select>
                  </div>
                ...
                <div className={`p-5 rounded-3xl space-y-4 ${theme !== 'light' ? 'bg-[var(--bg-page)]/40 border border-[var(--border-subtle)]' : 'bg-slate-50 border border-slate-100'}`}>
                  {accumulatedChallengePnL !== 0 && (
                    <div className="flex justify-between items-center opacity-60">
                      ...
                      <span className="text-xs font-black text-blue-400">+${accumulatedChallengePnL.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                    </div>
                  )}

                  <div className={`flex justify-between items-center border-t pt-3 ${theme !== 'light' ? 'border-[var(--border-subtle)]' : 'border-slate-200/50'}`}>
                    ...
                    <span className="text-base font-black text-emerald-500">+${netWithdrawals.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                  </div>

                  <div className={`flex justify-between items-center border-t pt-3 ${theme !== 'light' ? 'border-[var(--border-subtle)]' : 'border-slate-200/50'}`}>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-black uppercase text-emerald-400">Prop Day Threshold</span>
                      <Target size={10} className="text-emerald-500/50" />
                    </div>
                    {isEditing ? (
                      <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                        <input type="number" value={editFormData.propThreshold} onChange={e => setEditFormData({ ...editFormData, propThreshold: Number(e.target.value) })} className={`w-16 text-right px-2 py-1 text-xs font-black rounded outline-none focus:border-blue-500 ${theme !== 'light' ? 'bg-[var(--bg-input)] text-emerald-400 border border-[var(--border-subtle)]' : 'bg-white border border-slate-200'}`} />
                        <span className="text-xs font-black text-slate-500">$</span>
                      </div>
                    ) : (
                      <span className="text-sm font-black text-emerald-500">${acc.propThreshold || 150}</span>
                    )}
                  </div>
                </div>
              </div>

              <div className={`grid grid-cols-2 gap-4 pt-4 border-t ${theme !== 'light' ? 'border-[var(--border-subtle)]' : 'border-slate-100'}`}>
                ...
                <div className="text-right">
                  ...
                  <p className={`font-black text-sm ${netAccResult >= 0 ? 'text-blue-500' : 'text-rose-600'}`}>
                    {netAccResult >= 0 ? '+' : ''}${netAccResult.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </p>
                </div>
              </div>
              ...
              {/* Background Art */}
              <div className={`absolute -bottom-6 -right-6 rotate-12 group-hover:scale-110 pointer-events-none transition-all duration-700 ${theme !== 'light' ? 'text-[var(--bg-page)] opacity-20' : 'text-slate-800/5'}`}>
      ...
      {/* PASS Confirmation Modal */}
      {
        passConfirmTarget && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300" onClick={(e) => { e.stopPropagation(); setPassConfirmTarget(null); }}>
            <div className={`max-w-md w-full p-10 rounded-[48px] border shadow-2xl ${theme !== 'light' ? 'bg-[var(--bg-card)] border-[var(--border-subtle)]' : 'bg-white border-blue-200'}`} onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className={`text-3xl font-black italic tracking-tighter ${theme !== 'light' ? 'text-white' : 'text-slate-900'}`}>CHALLENGE PASSED!</h3>
                ...
                <div className="flex gap-4 w-full pt-4">
                  <button
                    ...
                    className={`flex-1 py-5 rounded-2xl font-black uppercase text-xs tracking-widest transition-all ${theme !== 'light' ? 'bg-[var(--bg-page)] text-slate-300 hover:bg-[var(--bg-input)]' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                  >
                    Zrušit
                  </button>
                  <button
      ...
      {/* Payout Modal */}
      {
        payoutTarget && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setPayoutTarget(null)}>
            <div className={`max-w-md w-full p-10 rounded-[48px] border shadow-2xl ${theme !== 'light' ? 'bg-[var(--bg-card)] border-[var(--border-subtle)]' : 'bg-white border-emerald-200'}`} onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center text-center space-y-6">
                <div className="p-6 bg-emerald-500/10 text-emerald-500 rounded-full border border-emerald-500/20 shadow-xl">
                  <HandCoins size={48} />
                </div>
                <div className="space-y-2">
                  <h3 className={`text-3xl font-black italic tracking-tighter ${theme !== 'light' ? 'text-white' : 'text-slate-900'}`}>RECORD PAYOUT</h3>
                  ...
                  <div className="space-y-2 text-left">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="1000"
                        className={`w-full pl-12 pr-4 py-4 rounded-2xl border text-xl font-black font-mono outline-none ${theme !== 'light' ? 'bg-[var(--bg-input)] border-[var(--border-subtle)] text-white' : 'bg-slate-50 border-slate-200'}`}
                      />
                    </div>
                  </div>
      ...
      {/* FAIL Confirmation Modal */}
      {
        failConfirmTarget && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300" onClick={(e) => { e.stopPropagation(); setFailConfirmTarget(null); }}>
            <div className={`max-w-md w-full p-10 rounded-[48px] border shadow-2xl ${theme !== 'light' ? 'bg-[var(--bg-card)] border-[var(--border-subtle)]' : 'bg-white border-rose-200'}`} onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center text-center space-y-6">
                <div className="p-6 bg-rose-500/10 text-rose-500 rounded-full border border-rose-500/20 shadow-xl shadow-rose-500/10">
                  <Skull size={48} />
                </div>
                <div className="space-y-2">
                  <h3 className={`text-3xl font-black italic tracking-tighter ${theme !== 'light' ? 'text-white' : 'text-slate-900'}`}>CHALLENGE FAILED?</h3>
                  <p className="text-slate-500 font-medium text-sm leading-relaxed px-4">
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setAccountToDelete(null)}>
            <div className={`max-w-md w-full p-10 rounded-[48px] border shadow-2xl ${theme !== 'light' ? 'bg-[var(--bg-card)] border-[var(--border-subtle)]' : 'bg-white border-rose-200'}`} onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className={`text-3xl font-black italic tracking-tighter ${theme !== 'light' ? 'text-white' : 'text-slate-900'}`}>ERASE CAPITAL?</h3>
                  <p className="text-slate-500 font-medium text-sm leading-relaxed px-4">
