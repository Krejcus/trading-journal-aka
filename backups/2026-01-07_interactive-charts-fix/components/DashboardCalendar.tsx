         ...
         <div className={`px-6 py-6 border-b flex flex-col md:flex-row justify-between md:items-center gap-4 ${theme !== 'light' ? 'border-white/5' : 'border-slate-100 bg-slate-50/80'}`}>
            ...
            <div className={`flex flex-col items-end gap-1 px-5 py-3 rounded-2xl border ${theme === 'oled' ? 'bg-black border-white/10' :
               ...
               <span className={`text-2xl font-mono font-black tracking-tighter ${totalMonthPnl >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                  {totalMonthPnl >= 0 ? '+' : '-'}${Math.abs(totalMonthPnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}
               </span>
            </div>
         </div>
...
const CalendarCell: React.FC<{ cell: GridCell; theme: 'dark' | 'light' | 'oled'; maxPnL: number; onDayClick: (day: DayData) => void; onWeekClick: (week: WeekData) => void; }> = ({ cell, theme, maxPnL, onDayClick, onWeekClick }) => {
   ...
   return (
      <div onClick={() => onDayClick(day)} className={`aspect-square rounded-2xl p-2 md:p-3 flex flex-col justify-between cursor-pointer border relative overflow-hidden transition-all group hover:ring-2 hover:ring-slate-500/30 ${borderClass} ${theme === 'oled' ? 'bg-black hover:bg-white/5 shadow-none' :
         ...
         <div className="flex-1 flex items-center justify-center">
            {day.hasTrades ? <span className={`font-black text-sm md:text-base tracking-tighter ${intensity > 0.6 ? 'text-white' : (day.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500')}`}>${Math.abs(day.pnl) >= 1000 ? (Math.abs(day.pnl) / 1000).toFixed(1) + 'k' : Math.abs(day.pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span> : null}
         </div>
...
const WeekDetailModal: React.FC<{ week: WeekData; monthName: string; theme: 'dark' | 'light' | 'oled'; onClose: () => void; accounts: Account[]; emotions: CustomEmotion[] }> = ({ week, monthName, theme, onClose, accounts, emotions }) => {
               ...
               <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                  <div className={`w-full lg:w-[40%] overflow-y-auto custom-scrollbar border-r p-6 flex flex-col gap-6 ${isDark ? 'bg-[#0F172A]/50 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                     <div className="grid grid-cols-2 gap-4">
                        ...
                        <StatCard icon={<Trophy size={16} />} label="Max Win" value={maxWin ? `+$${maxWin.pnl.toLocaleString(undefined, { maximumFractionDigits: 0 })}` : '-'} color="text-emerald-400" subValue={maxWin?.instrument} />
                        <StatCard icon={<Trophy size={16} />} label="Max Loss" value={maxLoss ? `-$${Math.abs(maxLoss.pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : '-'} color="text-rose-400" subValue={maxLoss?.instrument} />
                     </div>
                  </div>
                           ...
                           <div className="space-y-2">
                              {week.days.map((day) => (
                                 <div key={day.dateStr} className={`p-4 rounded-2xl border flex items-center justify-between transition-all hover:scale-[1.01] ${isDark ? 'bg-[#0a0f1d] border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
                                    ...
                                    <div className="text-right"><span className={`text-lg font-black font-mono ${day.hasTrades ? (day.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500') : 'text-slate-500'}`}>${Math.abs(day.pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></div>
                                 </div>
                              ))}
                           </div>
                        ) : (
                           <div className="space-y-2">
                              {allWeekTrades.length > 0 ? allWeekTrades.map((trade) => (
                                 <div key={trade.id} onClick={() => setSelectedTrade(trade)} className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all hover:scale-[1.01] hover:border-blue-500/30 ${isDark ? 'bg-[#0a0f1d] border-white/5' : 'bg-white border-slate-200'} ${trade.executionStatus === 'Missed' ? 'opacity-60' : ''}`}>
                                    ...
                                    <div className="text-right"><span className={`text-sm font-black font-mono ${trade.executionStatus === 'Missed' ? 'text-blue-400' : (trade.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500')}`}>{trade.executionStatus === 'Missed' ? '±' : (trade.pnl >= 0 ? '+' : '')}${Math.abs(trade.pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span></div>
                                 </div>
...
const DayDeepDiveModal: React.FC<{ day: DayData; theme: 'dark' | 'light' | 'oled'; onClose: () => void; accounts: Account[]; emotions: CustomEmotion[] }> = ({ day, theme, onClose, accounts, emotions }) => {
                  ...
                  <div className="flex items-center gap-6">
                     <div className="text-right">
                        ...
                        <p className={`text-3xl font-black font-mono leading-none ${hasTrades ? (pnl >= 0 ? 'text-emerald-500' : 'text-rose-500') : 'text-slate-500'}`}>
                           {hasTrades ? (pnl >= 0 ? '+' : '-') : ''}${Math.abs(pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                        </p>
                     </div>
                     <button onClick={onClose} className="p-2.5 hover:bg-white/10 rounded-full transition-all text-slate-500 hover:text-white"><X size={24} /></button>
                  </div>
                           ...
                           <div className="space-y-2">
                              {trades.map((t, i) => (
                                 <div key={i} onClick={() => setSelectedTrade(t)} className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all hover:scale-[1.02] ${isDark ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-slate-100 hover:shadow-md'} ${t.executionStatus === 'Missed' ? 'opacity-60' : ''}`}>
                                    ...
                                    <div className="flex items-center gap-3"><span className={`text-sm font-black font-mono ${t.executionStatus === 'Missed' ? 'text-blue-400' : (t.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500')}`}>{t.executionStatus === 'Missed' ? '±' : (t.pnl >= 0 ? '+' : '')}${Math.abs(t.pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}</span><ArrowRight size={12} className="text-slate-600" /></div>
                                 </div>
                              ))}
                           </div>
                        )}
...
interface SingleMonthViewProps { year: number; month: number; trades: Trade[]; preps: DailyPrep[]; reviews: DailyReview[]; theme: 'dark' | 'light' | 'oled'; onPrev: () => void; onNext: () => void; canPrev: boolean; canNext: boolean; onDayClick: (day: DayData) => void; onWeekClick: (week: WeekData) => void; }
