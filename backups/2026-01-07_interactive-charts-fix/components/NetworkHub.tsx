...
const NetworkHub: React.FC<NetworkHubProps> = ({ theme, accounts, emotions }) => {
   ...
   const enterSpectatorMode = async (userId: string) => {
      ...
      setLoading(true);
      try {
         const [profile, trades, remoteAccounts, preps, reviews, remotePrefs] = await Promise.all([
         ...
         setSpectatorData({ trades, accounts: activeAccounts, preps, reviews, preferences: remotePrefs });
                        ...
                        <div className="space-y-3">
                           ...
                           {(selectedTrade.screenshots?.length || selectedTrade.screenshot) ? (
                              ...
                              <div className={`p-8 rounded-2xl border border-dashed text-center flex flex-col items-center justify-center gap-2 ${isDark ? 'border-[var(--border-subtle)] bg-[var(--bg-page)]/20' : 'border-slate-200 bg-slate-50'}`}>
                                 ...
                                 <p className={`text-[9px] font-black uppercase ${isDark ? 'text-slate-700' : 'text-slate-400'}`}>No Visuals</p>
                              </div>
                           )}
                        </div>
                     </div>
