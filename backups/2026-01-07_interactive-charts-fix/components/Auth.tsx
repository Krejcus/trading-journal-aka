...
const Auth: React.FC<AuthProps> = ({ onLogin, theme }) => {
  ...
  const inputClass = `w-full pl-10 pr-4 py-3 rounded-xl border transition-all outline-none focus:ring-2 focus:ring-blue-500/40 ${theme !== 'light' ? 'bg-slate-900 border-slate-800 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'
    }`;

  if (showCheckEmail) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-[var(--bg-page)] relative overflow-hidden text-center transition-colors duration-300">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-blue-600/10 rounded-full blur-[120px] animate-pulse"></div>
        </div>

        <div className="w-full max-w-md relative z-10 p-8 rounded-[40px] border shadow-2xl theme-card backdrop-blur-xl">
          ...
          <h2 className="text-3xl font-black text-[var(--text-primary)] tracking-tighter mb-4 uppercase italic">Zkontroluj si e-mail!</h2>
          <p className="theme-text-secondary font-medium mb-8 leading-relaxed">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-[var(--bg-page)] relative overflow-hidden transition-colors duration-300">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-black text-[var(--text-primary)] tracking-tighter mb-2 italic uppercase">ALPHATRADE</h1>
        ...
        <div className={`p-8 rounded-[40px] border shadow-2xl theme-card backdrop-blur-xl`}>
          {(authMode === 'login' || authMode === 'signup') && (
            <div className={`flex gap-4 mb-8 p-1 rounded-2xl border theme-border ${theme !== 'light' ? 'bg-slate-950/50' : 'bg-slate-100'}`}>
              <button
                onClick={() => setAuthMode('login')}
                className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${authMode === 'login' ? 'bg-blue-600 text-white' : 'theme-text-secondary'}`}
                type="button"
              >
                Přihlášení
              </button>
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${authMode === 'signup' ? 'bg-blue-600 text-white' : 'theme-text-secondary'}`}
                type="button"
              >
                Registrace
              </button>
        ...
        <div className="mt-10 grid grid-cols-3 gap-4">
          {[
            ...
            <div key={i} className="flex flex-col items-center gap-1">
              <div className="p-2 rounded-lg bg-[var(--bg-sidebar)] theme-text-secondary border theme-border">
                <item.icon size={16} />
              </div>
              <span className="text-[8px] font-bold theme-text-secondary uppercase">{item.label}</span>
            </div>
          ))}
        </div>
      </div>
