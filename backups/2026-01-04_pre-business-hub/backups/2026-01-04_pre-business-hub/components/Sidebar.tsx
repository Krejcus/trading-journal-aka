...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const sidebarClasses = `
    ...
    ${isDark ? 'bg-slate-950/40 border-r border-white/10 backdrop-blur-2xl' : 'bg-white/80 border-r border-slate-200 shadow-xl backdrop-blur-xl'}
    flex flex-col
  `;
  ...
  const navItemClass = (isActive: boolean) => `
    ...
    ${isActive 
      ? 'bg-white/10 border-white/20 text-white shadow-[0_0_15px_rgba(255,255,255,0.05)] backdrop-blur-md' 
      ...
      <aside className={sidebarClasses}>
        ...
        <div className="flex-shrink-0 relative">
          ...
          <button 
            ...
            className={`hidden lg:flex absolute -right-3 top-10 z-[70] w-6 h-6 items-center justify-center rounded-full border shadow-2xl transition-all hover:scale-110 active:scale-90
            ...
            <div className="flex items-center gap-4 overflow-hidden">
              ...
              {!isCollapsed && (
                <div className="animate-in fade-in slide-in-from-left-4 duration-500">
                  <h1 className="text-white font-black text-xl tracking-tighter italic uppercase leading-none">AlphaTrade</h1>
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-[0.3em] mt-1">Terminal Hub</p>
                </div>
              )}
            </div>
            <button onClick={() => setIsOpen(false)} className="lg:hidden p-2 hover:bg-white/10 rounded-full text-slate-400"><X size={24} /></button>
          </div>
        ...
        <nav className="flex-1 px-2 space-y-2 overflow-y-auto custom-scrollbar no-scrollbar py-2">
          {!isCollapsed && <p className="px-6 text-[9px] font-black uppercase text-slate-600 mb-3 mt-4 tracking-[0.3em] animate-in fade-in duration-500">Analytika</p>}
          {isCollapsed && <div className="h-px bg-white/5 mx-6 my-4" />}
          ...
          {mainItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon size={isCollapsed ? 20 : 16} className={isActive ? 'text-white' : 'text-current'} />
                {!isCollapsed && <span className="animate-in fade-in duration-500 whitespace-nowrap">{item.label}</span>}
                {isActive && !isCollapsed && <div className="absolute right-4 w-1.5 h-1.5 bg-white rounded-full shadow-[0_0_8px_#fff]"></div>}
              </button>
            );
          })}

          {!isCollapsed && <p className="px-6 text-[9px] font-black uppercase text-slate-600 mb-3 mt-10 tracking-[0.3em] animate-in fade-in duration-500">Konfigurace</p>}
          {isCollapsed && <div className="h-px bg-white/5 mx-6 my-8" />}
        ...
        <div className={`p-4 mt-auto border-t transition-colors ${isDark ? 'border-white/5' : 'border-slate-200'} ${isCollapsed ? 'lg:p-2' : ''}`}>
           <div 
             ...
             className={`flex items-center justify-between gap-3 p-4 rounded-[24px] transition-all relative overflow-hidden group cursor-pointer
             ${isDark ? 'bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/20' : 'bg-slate-50 border border-slate-100 hover:shadow-md'} 
             ${isCollapsed ? 'lg:p-3 lg:justify-center' : ''}`}
           >
              <div className="flex items-center gap-4 min-w-0 relative z-10">
                <div className="w-10 h-10 rounded-2xl border border-white/10 overflow-hidden flex-shrink-0 shadow-inner">
                  {user.avatar ? (
