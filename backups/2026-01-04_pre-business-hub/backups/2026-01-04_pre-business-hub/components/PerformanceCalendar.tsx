  ...
  return (
    <div className="space-y-8">
      {months.map(({ key, days }) => {
        ...
        return (
          <div key={key} className={`rounded-[32px] p-6 border transition-colors ${
            theme === 'dark' ? 'bg-[#0a0f1d]/90 border-white/5 backdrop-blur-xl' : 'bg-white border-slate-200 shadow-sm'
          }`}>
            <div className="flex justify-between items-center mb-4">
              <h4 className={`text-xl font-black capitalize tracking-tighter ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{monthName}</h4>
              ...
              {days.map((day) => (
                <div 
                  ...
                  className={`
                    ...
                    ${day.pnl > 0 
                      ? (theme === 'dark' ? 'bg-emerald-500/10 border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.05)]' : 'bg-emerald-50 border-emerald-200') 
                      : (theme === 'dark' ? 'bg-rose-500/10 border-rose-500/20 shadow-[0_0_15px_rgba(244,63,94,0.05)]' : 'bg-rose-50 border-rose-200')
                    }
                  `}
                >
                  <div className="flex justify-between items-start">
                    <span className={`text-[10px] font-mono font-black ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>{day.date.split('-')[2]}</span>
                    <span className={`text-[8px] font-black px-1.5 py-0.5 rounded border ${
                      theme === 'dark' ? 'bg-slate-900 border-white/5 text-slate-500' : 'bg-white text-slate-500 shadow-sm border-slate-100'
                    }`}>{day.trades}x</span>
                  </div>
