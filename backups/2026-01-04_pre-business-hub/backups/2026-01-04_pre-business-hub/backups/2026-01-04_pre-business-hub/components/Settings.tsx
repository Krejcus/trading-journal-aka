...
import { CustomEmotion, SessionConfig, IronRule } from '../types';
...
const Settings: React.FC<SettingsProps> = ({
  ...
  const inputClass = `px-4 py-3 rounded-xl border focus:ring-2 focus:ring-blue-500/40 outline-none transition-all ${isDark ? 'bg-slate-950 border-slate-800 text-white placeholder-slate-700' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400'
    }`;
  ...
  return (
    <div className={`p-0.5 gap-0.5 flex flex-col lg:flex-row animate-in fade-in slide-in-from-bottom-4 duration-700 pb-16 max-w-7xl mx-auto rounded-[32px] border overflow-hidden ${isDark ? 'bg-slate-900/40 border-white/5 backdrop-blur-3xl' : 'bg-white border-slate-200 shadow-2xl'}`}>
      {/* Integrated Sidebar Navigation */}
      <aside className={`w-full lg:w-64 shrink-0 p-4 lg:p-6 flex flex-col border-b lg:border-b-0 lg:border-r ${isDark ? 'border-white/5 bg-slate-950/20' : 'border-slate-100 bg-slate-50/50'}`}>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3.5 px-5 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all relative group
                  ${isActive
                    ...
                    : (isDark ? 'text-slate-500 hover:text-slate-300 hover:bg-white/5' : 'text-slate-400 hover:text-slate-900 hover:bg-white')
                  }
                `}
              >
        ...
        <div className="space-y-8">
          {activeTab === 'psychology' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
              {/* Errors Section */}
              <section className={`p-6 rounded-[32px] border relative overflow-hidden ${isDark ? 'bg-[#0a0f1d]/80 border-rose-500/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
                  {userMistakes.map(mistake => (
                    <div key={mistake} className={`p-4 rounded-2xl border flex items-center justify-between group transition-all hover:scale-[1.01] ${isDark ? 'bg-slate-950/50 border-white/5 hover:border-rose-500/30' : 'bg-slate-50 border-slate-100 hover:shadow-sm'}`}>
              ...
              {/* Emotions Section */}
              <section className={`p-6 rounded-[32px] border relative overflow-hidden ${isDark ? 'bg-[#0a0f1d]/80 border-purple-500/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                <div className="flex items-center gap-3 mb-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
                  {userEmotions.map(emo => (
                    <div key={emo.id} className={`p-4 rounded-2xl border flex flex-col items-center justify-center gap-2 group transition-all hover:scale-[1.02] ${isDark ? 'bg-slate-950/50 border-white/5 hover:border-purple-500/30' : 'bg-slate-50 border-slate-100 hover:shadow-sm'}`}>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
              ...
              <section className={`p-6 rounded-[32px] border relative overflow-hidden ${isDark ? 'bg-[#0a0f1d]/80 border-blue-500/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className="space-y-2 mb-6">
                  {ironRules.map(rule => (
                    <div key={rule.id} className={`p-4 rounded-2xl border flex items-center justify-between group transition-all ${isDark ? 'bg-slate-950/50 border-white/5 hover:border-blue-500/30' : 'bg-slate-50 border-slate-100 hover:shadow-sm'}`}>
          ...
          {activeTab === 'market' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
              <section className={`p-6 rounded-[32px] border relative overflow-hidden ${isDark ? 'bg-[#0a0f1d]/80 border-indigo-500/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className={`mb-8 p-6 rounded-[32px] border ${isDark ? 'bg-slate-950/50 border-white/5' : 'bg-slate-50 border-slate-100 shadow-inner'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {sessions.map(session => (
                    <div key={session.id} className={`p-5 rounded-[32px] border relative group transition-all hover:scale-[1.01] ${isDark ? 'bg-slate-950/80 border-white/5 hover:border-indigo-500/20' : 'bg-white border-slate-100 shadow-sm hover:shadow-lg'}`}>
          ...
          {activeTab === 'financials' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
              <section className={`p-6 rounded-[32px] border relative overflow-hidden ${isDark ? 'bg-[#0a0f1d]/80 border-emerald-500/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mb-8 p-6 rounded-[24px] bg-slate-950/20 border border-white/5 backdrop-blur-xl">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  {Object.entries(instrumentFees).map(([inst, fee]) => (
                    <div key={inst} className={`p-5 rounded-[24px] border flex flex-col items-center gap-1.5 relative group transition-all hover:scale-[1.05] ${isDark ? 'bg-slate-950/80 border-white/5 hover:border-emerald-500/30' : 'bg-white border-slate-100 shadow-sm hover:shadow-lg'}`}>
