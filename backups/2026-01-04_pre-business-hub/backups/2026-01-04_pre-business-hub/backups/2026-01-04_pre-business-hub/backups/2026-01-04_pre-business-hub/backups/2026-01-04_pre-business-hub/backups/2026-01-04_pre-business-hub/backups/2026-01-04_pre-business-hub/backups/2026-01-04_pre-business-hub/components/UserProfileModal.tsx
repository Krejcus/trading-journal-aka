...
const UserProfileModal: React.FC<UserProfileModalProps> = ({ user, trades, isOpen, onClose, onUpdate, theme }) => {
  ...
  const inputClass = `w-full px-4 py-3 rounded-xl border outline-none transition-all focus:ring-2 focus:ring-blue-500/40 ${
    isDark ? 'bg-slate-950/50 border-slate-800 text-white placeholder-slate-600' : 'bg-slate-50 border-slate-200 text-slate-900'
  }`;

  const disabledInputClass = `w-full px-4 py-3 rounded-xl border outline-none cursor-not-allowed opacity-60 ${
