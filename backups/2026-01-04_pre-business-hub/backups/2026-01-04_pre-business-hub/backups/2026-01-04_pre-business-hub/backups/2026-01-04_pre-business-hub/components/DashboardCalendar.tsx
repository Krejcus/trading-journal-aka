...
const Tooltip: React.FC<{ text: string; theme: 'dark' | 'light'; subtext?: string }> = ({ text, theme, subtext }) => (
   <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-48 p-3 rounded-2xl border shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-2xl z-[400] opacity-0 pointer-events-none transition-all duration-200 translate-y-2 scale-95 group-hover:opacity-100 group-hover:translate-y-0 group-hover:scale-100 ${theme === 'dark' ? 'bg-slate-900/95 border-slate-700 text-slate-200' : 'bg-white/95 border-slate-200 text-slate-700'
      ...
      <div className={`absolute top-full left-1/2 -translate-x-1/2 border-8 border-transparent ${theme === 'dark' ? 'border-t-slate-700' : 'border-t-slate-200'}`}></div>
   </div>
);

const InfoIcon: React.FC<{ text: string; theme: 'dark' | 'light' }> = ({ text, theme }) => (
...
const DashboardCalendar: React.FC<DashboardCalendarProps> = ({
   ...
   if (monthsData.length === 0) return (
      <div className={`p-8 text-center rounded-xl border border-dashed ${theme === 'dark' ? 'border-slate-700 text-slate-500' : 'border-slate-300 text-slate-400'}`}>
   ...
   return (
      <div className="relative h-full flex flex-col">
         ...
         {selectedDay && createPortal(
            <DayDeepDiveModal day={selectedDay} theme={theme} onClose={() => setSelectedDay(null)} accounts={accounts} emotions={emotions} />,
            document.body
         )}
         {selectedWeek && createPortal(
            <WeekDetailModal week={selectedWeek} monthName={new Date(currentData.year, currentData.month - 1, 1).toLocaleString('cs-CZ', { month: 'long' })} theme={theme} onClose={() => setSelectedWeek(null)} accounts={accounts} emotions={emotions} />,
            document.body
         )}
      </div>
   );
...
const SingleMonthView: React.FC<SingleMonthViewProps> = ({ year, month, trades, preps, reviews, theme, onPrev, onNext, canPrev, canNext, onDayClick, onWeekClick }) => {
   ...
   return (
      <div className={`rounded-[32px] border shadow-sm overflow-hidden transition-all flex flex-col h-full ${theme === 'dark' ? 'bg-[#0a0f1d]/90 border-white/5 shadow-2xl backdrop-blur-xl' : 'bg-white border-slate-200'}`}>
         <div className={`px-6 py-6 border-b flex flex-col md:flex-row justify-between md:items-center gap-4 ${theme === 'dark' ? 'border-white/5' : 'border-slate-100 bg-slate-50/80'}`}>
            <div className="flex flex-col gap-4">
               <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
               ...
               <div className="flex items-center gap-3 select-none ml-0.5">
                  <button onClick={onPrev} disabled={!canPrev} className={`p-1.5 rounded-xl transition-all ${!canPrev ? 'opacity-20 pointer-events-none' : (theme === 'dark' ? 'bg-white/5 hover:bg-white/10 text-slate-300 border border-white/5' : 'bg-slate-100 hover:bg-slate-200')}`}><ChevronLeft size={18} /></button>
                  <div className="min-w-[130px]"><h3 className={`text-xl font-black capitalize leading-none tracking-tighter ${theme === 'dark' ? 'text-white' : 'text-blue-600'}`}>{monthName}</h3></div>
                  <button onClick={onNext} disabled={!canNext} className={`p-1.5 rounded-xl transition-all ${!canNext ? 'opacity-20 pointer-events-none' : (theme === 'dark' ? 'bg-white/5 hover:bg-white/10 text-slate-300 border border-white/5' : 'bg-slate-100 hover:bg-slate-200')}`}><ChevronRight size={18} /></button>
               </div>
            </div>
            <div className={`flex flex-col items-end gap-1 px-5 py-3 rounded-2xl border ${theme === 'dark' ? 'bg-slate-900/50 border-white/5' : 'bg-white border-slate-200'}`}>
               ...
               <span className={`text-2xl font-mono font-black tracking-tighter ${totalMonthPnl >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                  {totalMonthPnl >= 0 ? '+' : '-'}${Math.abs(totalMonthPnl).toLocaleString()}
               </span>
            </div>
         </div>
         ...
         <div className="grid grid-cols-6 gap-3 p-4 pb-0">
            {DAY_LABELS.map((label, idx) => (
               <div key={label} className={`py-2 rounded-xl text-center text-[10px] font-black uppercase tracking-widest border transition-colors ${idx === 5
                  ...
                  : (theme === 'dark' ? 'text-slate-500 bg-white/5 border-white/5' : 'text-slate-400 bg-slate-50 border-slate-100')
                  }`}>
                  {label}
               </div>
            ))}
...
const CalendarCell: React.FC<{ cell: GridCell; theme: 'dark' | 'light'; maxPnL: number; onDayClick: (day: DayData) => void; onWeekClick: (week: WeekData) => void; }> = ({ cell, theme, maxPnL, onDayClick, onWeekClick }) => {
   ...
   if (cell.type === 'summary') {
      ...
      return (
         <div onClick={() => onWeekClick(week)} className={`aspect-square rounded-2xl flex flex-col items-center justify-center p-2 border transition-all cursor-pointer relative overflow-hidden group hover:ring-2 hover:ring-blue-500/50 ${theme === 'dark' ? 'bg-white/5 border-white/5 text-slate-400 hover:bg-white/10' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100'}`}>
            ...
            <span className={`font-black font-mono text-xs md:text-sm ${pnl >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>${Math.abs(pnl) >= 1000 ? (pnl / 1000).toFixed(1) + 'k' : pnl.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
   ...
   let bgStyle = {};
   let borderClass = theme === 'dark' ? 'border-white/5' : 'border-slate-100';
   if (day.hasTrades) {
   ...
   return (
      <div onClick={() => onDayClick(day)} className={`aspect-square rounded-2xl p-2 md:p-3 flex flex-col justify-between cursor-pointer border relative overflow-hidden transition-all group hover:ring-2 hover:ring-slate-500/30 ${borderClass} ${theme === 'dark' ? 'bg-white/5 hover:bg-white/10' : 'bg-white shadow-sm hover:shadow-md'}`} style={bgStyle}>
         ...
         <div className="flex justify-end items-start"><span className={`font-mono text-xs font-black ${!day.hasTrades && (theme === 'dark' ? 'text-slate-700' : 'text-slate-300')}`}>{day.dayNum}</span></div>
         <div className="flex-1 flex items-center justify-center">
            {day.hasTrades ? <span className={`font-black text-sm md:text-base tracking-tighter ${intensity > 0.6 ? 'text-white' : (day.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500')}`}>${Math.abs(day.pnl) >= 1000 ? (Math.abs(day.pnl) / 1000).toFixed(0) + 'k' : Math.abs(day.pnl).toLocaleString()}</span> : null}
         </div>
...
const WeekDetailModal: React.FC<{ week: WeekData; monthName: string; theme: 'dark' | 'light'; onClose: () => void; accounts: Account[]; emotions: CustomEmotion[] }> = ({ week, monthName, theme, onClose, accounts, emotions }) => {
               ...
               <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                  <div className={`w-full lg:w-[40%] overflow-y-auto custom-scrollbar border-r p-6 flex flex-col gap-6 ${isDark ? 'bg-[#0F172A]/50 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                     <div className="grid grid-cols-2 gap-4">
                        ...
                        <StatCard icon={<Trophy size={16} />} label="Max Win" value={maxWin ? `+$${maxWin.pnl.toLocaleString()}` : '-'} color="text-emerald-400" subValue={maxWin?.instrument} />
                        <StatCard icon={<AlertTriangle size={16} />} label="Max Loss" value={maxLoss ? `-$${Math.abs(maxLoss.pnl).toLocaleString()}` : '-'} color="text-rose-400" subValue={maxLoss?.instrument} />
                     </div>
                  </div>
                           ...
                           <div className="space-y-2">
                              {week.days.map((day) => (
                                 <div key={day.dateStr} className={`p-4 rounded-2xl border flex items-center justify-between transition-all hover:scale-[1.01] ${isDark ? 'bg-[#0a0f1d] border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
                                    ...
                                    <div className="text-right"><span className={`text-lg font-black font-mono ${day.hasTrades ? (day.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500') : 'text-slate-500'}`}>${Math.abs(day.pnl).toLocaleString()}</span></div>
                                 </div>
                              ))}
                           </div>
                        ) : (
                           <div className="space-y-2">
                              {allWeekTrades.length > 0 ? allWeekTrades.map((trade) => (
                                 <div key={trade.id} onClick={() => setSelectedTrade(trade)} className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all hover:scale-[1.01] hover:border-blue-500/30 ${isDark ? 'bg-[#0a0f1d] border-white/5' : 'bg-white border-slate-200'} ${trade.executionStatus === 'Missed' ? 'opacity-60' : ''}`}>
                                    ...
                                    <div className="text-right"><span className={`text-sm font-black font-mono ${trade.executionStatus === 'Missed' ? 'text-blue-400' : (trade.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500')}`}>{trade.executionStatus === 'Missed' ? '±' : (trade.pnl >= 0 ? '+' : '')}${Math.abs(trade.pnl).toLocaleString()}</span></div>
                                 </div>
...
const DayDeepDiveModal: React.FC<{ day: DayData; theme: 'dark' | 'light'; onClose: () => void; accounts: Account[]; emotions: CustomEmotion[] }> = ({ day, theme, onClose, accounts, emotions }) => {
                  ...
                  <div className="flex items-center gap-6">
                     <div className="text-right">
                        ...
                        <p className={`text-3xl font-black font-mono leading-none ${hasTrades ? (pnl >= 0 ? 'text-emerald-500' : 'text-rose-500') : 'text-slate-500'}`}>
                           {hasTrades ? (pnl >= 0 ? '+' : '-') : ''}${Math.abs(pnl).toLocaleString()}
                        </p>
                     </div>
                     <button onClick={onClose} className="p-2.5 hover:bg-white/10 rounded-full transition-all text-slate-500 hover:text-white"><X size={24} /></button>
                  </div>
                           ...
                           <div className="space-y-2">
                              {trades.map((t, i) => (
                                 <div key={i} onClick={() => setSelectedTrade(t)} className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all hover:scale-[1.02] ${isDark ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-slate-100 hover:shadow-md'} ${t.executionStatus === 'Missed' ? 'opacity-60' : ''}`}>
                                    ...
                                    <div className="flex items-center gap-3"><span className={`text-sm font-black font-mono ${t.executionStatus === 'Missed' ? 'text-blue-400' : (t.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500')}`}>{t.executionStatus === 'Missed' ? '±' : (t.pnl >= 0 ? '+' : '')}${Math.abs(t.pnl).toLocaleString()}</span><ArrowRight size={12} className="text-slate-600" /></div>
                                 </div>
                              ))}
                           </div>
                        )}
...
const TradeDetailOverlay: React.FC<{ trade: Trade, theme: 'dark' | 'light', onClose: () => void, accounts: Account[], emotions: CustomEmotion[] }> = ({ trade, theme, onClose, accounts, emotions }) => {
   const isDark = theme === 'dark';
      ...
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300">
         <div className={`w-full max-w-7xl h-[85vh] rounded-[32px] overflow-hidden shadow-2xl flex flex-col border ${isDark ? 'bg-[#0a0f1d] border-white/10' : 'bg-white border-slate-200'}`}>
            <div className={`h-20 shrink-0 border-b flex items-center justify-between px-6 lg:px-8 ${isDark ? 'border-white/5 bg-[#0a0f1d]' : 'bg-white border-slate-100'}`}>
               ...
               <div className={`text-3xl font-black font-mono tracking-tighter ${pnlColor}`}>{isMissed ? '±' : (isWin ? '+' : '')}${Math.abs(trade.pnl).toLocaleString()}</div>
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
               <div className={`w-full lg:w-[30%] flex flex-col overflow-y-auto custom-scrollbar border-r ${isDark ? 'bg-[#0F172A]/50 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                  <div className={`grid grid-cols-2 border-b ${isDark ? 'border-white/5 bg-[#0a0f1d]' : 'border-slate-200 bg-white'}`}>
                     ...
                     <MetricCell label="Risk" value={`$${riskAmount}`} color="text-slate-400" />
...
interface SingleMonthViewProps { year: number; month: number; trades: Trade[]; preps: DailyPrep[]; reviews: DailyReview[]; theme: 'dark' | 'light'; onPrev: () => void; onNext: () => void; canPrev: boolean; canNext: boolean; onDayClick: (day: DayData) => void; onWeekClick: (week: WeekData) => void; }
