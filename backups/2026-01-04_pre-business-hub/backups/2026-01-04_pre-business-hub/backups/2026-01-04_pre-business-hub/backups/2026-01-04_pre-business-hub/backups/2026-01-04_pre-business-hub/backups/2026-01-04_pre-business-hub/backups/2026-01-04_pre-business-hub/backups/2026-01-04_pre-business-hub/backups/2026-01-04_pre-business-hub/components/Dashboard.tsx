...
const Tooltip: React.FC<{ text: string; theme: 'dark' | 'light'; subtext?: string; color?: string }> = ({ text, theme, subtext, color }) => (
  <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-48 p-3 rounded-2xl border shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-2xl z-[1000] opacity-0 pointer-events-none transition-all duration-200 translate-y-2 scale-95 group-hover:opacity-100 group-hover:translate-y-0 group-hover:scale-100 ${theme === 'dark' ? 'bg-slate-900/95 border-slate-700 text-slate-200' : 'bg-white/95 border-slate-200 text-slate-700'
    }`}>
...
const CustomKpiTooltip = ({ active, payload, theme }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className={`px-3 py-2 rounded-xl border shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95 duration-200 z-[1000] pointer-events-none ${isDark ? 'bg-slate-900/95 border-slate-700 text-white' : 'bg-white/95 border-slate-200 text-slate-900'
        }`}>
        ...
        <p className="text-sm font-black flex items-center gap-2">
          ...
          {Math.abs(payload[0].value).toLocaleString()} {data.unit || '$'}
        </p>
      </div>
    );
  }
...
const CustomEdgeTooltip = ({ active, payload, label, theme }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className={`p-4 rounded-2xl border shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-200 min-w-[180px] z-[1000] pointer-events-none ${isDark ? 'bg-slate-900/95 border-slate-700 text-white' : 'bg-white/95 border-slate-200 text-slate-900'
        }`}>
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className={`${COLORS.textProfit} font-black`}>+${profit.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-500 font-medium">Hrubá ztráta:</span>
            <span className={`${COLORS.textLoss} font-black`}>-${loss.toLocaleString()}</span>
          </div>
          <div className={`flex justify-between items-center pt-2 mt-1 border-t ${isDark ? 'border-slate-800/50' : 'border-slate-100'}`}>
...
const ProKpiCard: React.FC<{
      ...
      return (
        <div className="flex items-center justify-between w-full h-full pl-2 gap-6">
          <div className="flex flex-col items-start justify-center h-full min-w-0">
            <span className={`text-3xl lg:text-4xl font-black tracking-tighter leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {value}
            </span>
          </div>
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex gap-1 mt-2">
            <div className="relative group" onMouseEnter={() => { const idx = chartData.findIndex(d => d.fill === COLORS.profit); if (idx >= 0) setActiveIndex(idx); }} onMouseLeave={() => setActiveIndex(-1)}>
              ...
              <Tooltip text="Hrubý zisk" subtext={`$${(data.profit || 0).toLocaleString()}`} color={COLORS.profit} theme={theme} />
            </div>
            <div className="relative group" onMouseEnter={() => { const idx = chartData.findIndex(d => d.fill === COLORS.loss); if (idx >= 0) setActiveIndex(idx); }} onMouseLeave={() => setActiveIndex(-1)}>
              <div className="w-2 h-2 rounded-full bg-rose-500 cursor-help" />
              <Tooltip text="Hrubá ztráta" subtext={`$${(data.loss || 0).toLocaleString()}`} color={COLORS.loss} theme={theme} />
            </div>
          </div>
        </div>
      );
  ...
  return (
    <div className={`p-5 rounded-[24px] border flex flex-col justify-between h-full relative overflow-visible transition-all hover:scale-[1.02] hover:shadow-xl ${isDark ? 'bg-[#0a0f1d]/80 border-white/5 hover:border-white/10' : 'bg-white border-slate-100 shadow-sm'}`}>
      <div className="flex justify-between items-start mb-2">
        ...
        {icon && <div className={`p-1.5 rounded-lg ${isDark ? 'bg-white/5 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>{icon}</div>}
      </div>
      <div className="flex-1 flex flex-col items-center justify-center min-h-[60px]">
        {type === 'text' && (
          <div className="text-center">
            <p className={`text-2xl lg:text-3xl font-black tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>{value}</p>
...
const WinnersLosersWidget: React.FC<{ stats: TradeStats, theme: 'dark' | 'light' }> = ({ stats, theme }) => {
  const isDark = theme === 'dark';
  ...
  return (
    <div className={`p-6 rounded-[32px] border transition-all relative h-full flex flex-col justify-between overflow-visible ${isDark ? 'bg-[#0a0f1d]/90 border-white/5 backdrop-blur-xl' : 'bg-white border-slate-200 shadow-sm'}`}>
...
const PerformanceByMonthWidget: React.FC<{ monthlyData: MonthlyData[], theme: 'dark' | 'light' }> = ({ monthlyData, theme }) => {
  const isDark = theme === 'dark';
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  const [view, setView] = useState<'individual' | 'accum'>('individual');
  const [unit, setUnit] = useState<'pct' | 'val'>('pct');
  const getIntensity = (val: number) => {
    if (val === 0) return isDark ? 'bg-slate-900/50' : 'bg-slate-50';
    if (val > 0) {
  ...
  return (
    <div className={`p-6 rounded-[32px] border ${isDark ? 'bg-[#0a0f1d]/90 border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
        ...
        <table className="w-full min-w-[800px] border-separate border-spacing-2">
          <thead>
            <tr>
              ...
              {months.map(m => <th key={m} className="p-3 rounded-xl bg-slate-900/30 text-[10px] font-black uppercase text-slate-500">{m}</th>)}
          ...
          <tbody>
            {monthlyData.map(yearRow => (
              <tr key={yearRow.year}>
                <td className="p-3 rounded-xl bg-slate-900/50 text-[10px] font-black text-center text-slate-300">{yearRow.year}</td>
                {months.map((_, i) => {
...
const HourlyEdgeWidget: React.FC<{ data: TimeStat[], theme: 'dark' | 'light' }> = ({ data, theme }) => (
  <div className={`p-6 rounded-[32px] border border-blue-500/10 flex flex-col min-h-[380px] overflow-visible ${theme === 'dark' ? 'bg-[#0a0f1d]/90 border-white/5 backdrop-blur-xl' : 'bg-white border-slate-200 shadow-sm'}`}>
    ...
    <div className="w-full h-[260px] mt-auto relative">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} stackOffset="sign" margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
          ...
          <ReferenceLine y={0} stroke={theme === 'dark' ? '#475569' : '#cbd5e1'} strokeWidth={1} />
...
const DailyEdgeWidget: React.FC<{ data: TimeStat[], theme: 'dark' | 'light' }> = ({ data, theme }) => {
  const tradingDays = data.filter(d => d.label !== 'So' && d.label !== 'Ne');
  return (
    <div className={`p-6 rounded-[32px] border flex flex-col min-h-[380px] overflow-visible ${theme === 'dark' ? 'bg-[#0a0f1d]/90 border-white/5 backdrop-blur-xl' : 'bg-white border-slate-200 shadow-sm'}`}>
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2">
      ...
      <div className="w-full h-[260px] mt-auto relative">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart layout="vertical" data={tradingDays} stackOffset="sign" margin={{ top: 5, right: 50, left: 10, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke={theme === 'dark' ? '#334155' : '#e2e8f0'} opacity={0.3} />
            ...
            <RechartsTooltip content={<CustomEdgeTooltip theme={theme} />} cursor={{ fill: 'rgba(59, 130, 246, 0.05)' }} allowEscapeViewBox={{ x: true, y: true }} />
            <ReferenceLine x={0} stroke={theme === 'dark' ? '#475569' : '#cbd5e1'} strokeWidth={1} />
...
const SessionBreakdownWidget: React.FC<{ trades: any[], theme: 'dark' | 'light', configs: SessionConfig[] }> = ({ trades, theme, configs }) => {
  const now = new Date();
  const currentMinutes = now.getHours() * 60 + now.getMinutes();

  return (
    <div className={`p-4 lg:p-6 rounded-[24px] lg:rounded-[32px] border h-full flex flex-col overflow-visible ${theme === 'dark' ? 'bg-[#0a0f1d]/90 border-white/5 backdrop-blur-xl' : 'bg-white border-slate-200 shadow-sm'}`}>
      <div className="flex justify-between items-center mb-4 lg:mb-6">
        <h3 className={`text-xs lg:text-sm font-black uppercase tracking-widest flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 lg:gap-4 flex-1">
        {configs.map(cfg => {
          ...
          return (
            <div key={cfg.id} className={`p-4 lg:p-5 rounded-xl lg:rounded-2xl border transition-all ${isLive ? 'border-blue-500/20 bg-blue-500/5 ring-1 ring-blue-500/30' : (theme === 'dark' ? 'bg-slate-900/40 border-slate-800' : 'bg-slate-50 border-slate-100')}`}>
              <div className="flex justify-between items-start mb-2 lg:mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: sessionColor }} />
                    <p className={`text-[9px] lg:text-[10px] font-black uppercase tracking-widest ${isLive ? (theme === 'dark' ? 'text-white' : 'text-slate-900') : 'text-slate-500'}`}>{cfg.name}</p>
                  </div>
...
const Dashboard: React.FC<DashboardProps> = ({ stats, theme, preps, reviews, layout, sessions, ironRules, onUpdateLayout, isEditing, onCloseEdit, accounts, emotions }) => {
  ...
  const renderWidget = (id: string, config?: DashboardWidgetConfig) => {
    switch (id) {
      ...
      case 'kpi_pnl': return <ProKpiCard theme={theme} label="Net P&L" value={`$${stats.totalPnL.toLocaleString()}`} sampleSize={stats.totalTrades} info="Čistý zisk nebo ztráta po odečtení všech nákladů a poplatků." icon={<div className="bg-purple-100 text-purple-600 p-1 rounded-lg dark:bg-purple-500/20"><BarChart3 size={14} /></div>} />;
      ...
      case 'equity': return <Charts stats={stats} theme={theme} onlyEquity isEditing={isEditing} showDisciplinedCurve={config?.showDisciplinedCurve} onToggleDisciplined={() => toggleDisciplinedCurve('equity')} />;
    ...
    <div className={`relative min-h-screen transition-all duration-700 max-w-full overflow-x-hidden ${isEditing && isArmoryOpen ? 'lg:pr-[340px]' : ''}`}>
      <div className="space-y-6 lg:space-y-10 pb-24 relative z-10 w-full overflow-x-hidden">
        <div className="flex justify-between items-center">
          <div>
            <h2 className={`text-3xl lg:text-5xl font-black tracking-tighter italic ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>TRADING TERMINAL</h2>
              ...
              <div key={catName} className="space-y-4">
                ...
                <div className="space-y-4 pl-1">
                  {filtered.map(master => {
                    ...
                    return (
                      <div key={master.id} className={`group relative p-3 rounded-2xl border transition-all flex items-center justify-between gap-3 ${isActive ? 'bg-blue-600/10 border-blue-500/30' : (theme === 'dark' ? 'bg-slate-900/40 border-slate-800 hover:border-slate-700' : 'bg-slate-50 border-slate-200 hover:shadow-sm')}`}>
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          ...
                          <div className="min-w-0">
                            <h4 className={`text-[10px] font-black uppercase tracking-tight truncate ${isActive ? 'text-blue-400' : (theme === 'dark' ? 'text-white' : 'text-slate-900')}`}>{master.label}</h4>
