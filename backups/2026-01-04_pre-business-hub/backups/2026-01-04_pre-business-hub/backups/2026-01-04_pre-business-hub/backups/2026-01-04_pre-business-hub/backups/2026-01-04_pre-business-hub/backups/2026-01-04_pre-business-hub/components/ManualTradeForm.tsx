...
import {
  ...
  AlertOctagon, AlertTriangle, Plus, Trash2, Clock, DollarSign
} from 'lucide-react';
...
const ManualTradeForm: React.FC<ManualTradeFormProps> = ({
  ...
  useEffect(() => {
    if (editTrade) {
      setFormData({
        ...
        customMultiplier: '1',
        entryDate: getLocalISOString(new Date(editTrade.timestamp - (editTrade.durationMinutes * 60000))),
        exitDate: getLocalISOString(new Date(editTrade.timestamp)),
        ...
        positionSize: editTrade.positionSize?.toString() || '1',
        ...
        planAdherence: editTrade.planAdherence || 'Yes',
        executionStatus: editTrade.executionStatus || (editTrade.isValid === false ? 'Invalid' : 'Valid')
      });
    } else {
      // Check for draft in localStorage
      const draft = localStorage.getItem('alphatrade_trade_draft');
      if (draft) {
        try {
  ...
  // Save draft whenever formData changes (only if not editing)
  useEffect(() => {
    if (!editTrade) {
  ...
  const calculations = useMemo(() => {
    ...
    const feePerUnit = instrumentFees[formData.instrument] || 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAdd({
      id: editTrade ? editTrade.id : `manual_${Date.now()}`,
  ...
  const inputContainerClass = `relative h-[42px] rounded-xl border transition-all flex items-center overflow-hidden ${theme === 'dark' ? 'bg-slate-950/40 border-white/10 focus-within:border-blue-500/50' : 'bg-white/80 border-slate-200 focus-within:border-blue-500'}`;
  const inlineLabelClass = "px-3 text-[9px] font-black uppercase text-slate-500 whitespace-nowrap border-r border-white/5 h-full flex items-center bg-white/5 min-w-[85px]";
  const inputClass = "w-full px-3 py-2 bg-transparent text-sm font-black tabular-nums outline-none text-white placeholder-slate-700";
  ...
  return (
    <>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 md:p-4 bg-slate-950/60 backdrop-blur-2xl animate-in fade-in duration-500">
        <div className={`w-full max-w-[1400px] max-h-[96vh] rounded-[32px] md:rounded-[40px] shadow-[0_32px_128px_rgba(0,0,0,0.8)] border flex flex-col overflow-hidden animate-in zoom-in-95 duration-500 relative z-10 ${theme === 'dark' ? 'bg-[#0B1120] border-white/10' : 'bg-white border-slate-200'}`}>
          <div className={`px-5 py-4 md:px-8 md:py-5 border-b flex justify-between items-center bg-white/5 border-white/5 backdrop-blur-md shrink-0`}>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5 md:p-8 lg:p-10 bg-gradient-to-b from-transparent to-black/20">
            <div className="flex flex-col lg:flex-row gap-6 mb-8 justify-between items-start lg:items-center bg-slate-900/60 p-4 rounded-3xl border border-white/5">
              <div className="flex flex-wrap gap-4 md:gap-6 w-full relative">
                <div className="flex flex-col"><span className="text-[8px] font-black text-slate-500 uppercase">{formData.executionStatus === 'Missed' ? 'Ušlý zisk' : 'Výsledek (Net)'}</span><span className={`text-xl font-mono font-black ${calculations.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500'} ${formData.executionStatus === 'Missed' ? 'opacity-50' : ''}`}>${calculations.pnl.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 md:gap-8 items-start">
                  ...
                  <div className="relative group">
                    <div className={inputContainerClass}>
                      ...
                      <button type="button" onClick={() => setIsAccountOpen(!isAccountOpen)} className="w-full h-full flex items-center justify-between px-3 text-xs font-black text-white hover:bg-white/5">
                        {currentAcc?.name} <ChevronDown size={12} className="text-slate-600" />
                      </button>
                    </div>
                    {isAccountOpen && (<div className="absolute top-full left-0 right-0 mt-2 z-[200] rounded-xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl overflow-hidden animate-in fade-in duration-200">{accounts.map(acc => (<button key={acc.id} type="button" onClick={() => { setFormData({ ...formData, accountId: acc.id }); setIsAccountOpen(false); }} className="w-full px-4 py-3 text-[10px] font-black text-left hover:bg-blue-600 text-slate-400 hover:text-white uppercase border-b border-white/5 last:border-0">{acc.name}</button>))}</div>)}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative group">
                    ...
                    {isInstrumentOpen && (<div className="absolute top-full left-0 right-0 mt-2 z-[200] rounded-xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl overflow-hidden animate-in fade-in duration-200">{INSTRUMENTS.map(i => (<button key={i.id} type="button" onClick={() => { setFormData({ ...formData, instrument: i.id }); setIsInstrumentOpen(false); }} className="w-full px-4 py-3 text-[10px] font-black text-left hover:bg-blue-600 text-slate-400 hover:text-white uppercase border-b border-white/5 last:border-0">{i.name}</button>))}</div>)}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className={`rounded-2xl border overflow-hidden ${theme === 'dark' ? 'border-white/5 bg-white/5' : 'border-slate-200'}`}>
                      <button type="button" onClick={() => setExpandedSection(expandedSection === 'emotions' ? null : 'emotions')} className={`w-full px-4 py-3 flex items-center justify-between text-[10px] font-black uppercase tracking-widest ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-50'}`}><span className="flex items-center gap-2"><Brain size={12} className="text-purple-500" /> Emoce</span>{expandedSection === 'emotions' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
                    ...
                    <div className={`rounded-2xl border overflow-hidden ${theme === 'dark' ? 'border-white/5 bg-white/5' : 'border-slate-200'}`}>
                      <button type="button" onClick={() => setExpandedSection(expandedSection === 'htf' ? null : 'htf')} className={`w-full px-4 py-3 flex items-center justify-between text-[10px] font-black uppercase tracking-widest ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-50'}`}><span className="flex items-center gap-2"><Monitor size={12} className="text-blue-500" /> HTF Kontext</span>{expandedSection === 'htf' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
                    ...
                    <div className={`rounded-2xl border overflow-hidden ${theme === 'dark' ? 'border-white/5 bg-white/5' : 'border-slate-200'}`}>
                      <button type="button" onClick={() => setExpandedSection(expandedSection === 'ltf' ? null : 'ltf')} className={`w-full px-4 py-3 flex items-center justify-between text-[10px] font-black uppercase tracking-widest ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-50'}`}><span className="flex items-center gap-2"><Zap size={12} className="text-amber-500" /> LTF Trigger</span>{expandedSection === 'ltf' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
                    ...
                    <div className={`rounded-2xl border overflow-hidden ${theme === 'dark' ? 'border-white/5 bg-white/5' : 'border-slate-200'}`}>
                      <button type="button" onClick={() => setExpandedSection(expandedSection === 'mistakes' ? null : 'mistakes')} className={`w-full px-4 py-3 flex items-center justify-between text-[10px] font-black uppercase tracking-widest ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-50'}`}><span className="flex items-center gap-2"><AlertOctagon size={12} className="text-rose-500" /> Chyby Exekuce</span>{expandedSection === 'mistakes' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
          ...
          <div className={`p-5 md:p-8 shrink-0 border-t flex flex-col sm:flex-row gap-3 md:gap-6 bg-white/5 border-white/5 backdrop-blur-xl`}>
