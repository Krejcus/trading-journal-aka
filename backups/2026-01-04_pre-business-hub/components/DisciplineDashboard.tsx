...
const DisciplineDashboard: React.FC<DisciplineDashboardProps> = ({ theme, preps, reviews, trades, ironRules = [] }) => {
  const stats = useMemo(() => {
    ...
    allRuleIds.forEach(ruleId => {
      ...
      const label = ruleDef ? ruleDef.label : ruleId;
      const type = ruleDef ? ruleDef.type : (ruleId.startsWith('ritual') ? 'ritual' : 'trading');
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200';
    }
  };

  return (
    <div className={`p-5 rounded-[32px] border transition-all flex flex-col overflow-visible ${
        ...
        <div className="flex justify-between items-center mb-5 shrink-0">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900/50 border border-white/5">
             <Flame size={12} className={stats.currentStreak > 0 ? 'text-orange-500' : 'text-slate-600'} />
             <span className="text-[9px] font-black uppercase text-slate-400">Streak: <span className="text-white">{stats.currentStreak}</span></span>
          </div>
        </div>
            ...
            <div className="shrink-0 text-center">
              <div className="w-12 h-12 rounded-full border-4 border-slate-800 flex items-center justify-center relative">
                <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 64 64">
                   <circle cx="32" cy="32" r="28" fill="none" stroke={theme === 'dark' ? '#0f172a' : '#f1f5f9'} strokeWidth="4" />
                   <circle 
          ...
          <div className="flex-1">
             <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 xl:grid-cols-6 gap-2">
                {Object.entries(stats.ruleStreaks).map(([id, data]: [string, any]) => {
                  ...
                  return (
                    <div key={id} className={`p-2.5 rounded-xl border flex flex-col items-center justify-center gap-1.5 transition-all hover:bg-white/5 ${
                      theme === 'dark' ? 'bg-slate-900/30 border-white/5' : 'bg-slate-50 border-slate-100'
                    }`}>
                      {/* Mini Ring */}
                      <div className="relative w-8 h-8 flex items-center justify-center">
                         <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 50 50">
                            <circle cx="25" cy="25" r="22" fill="none" stroke={theme === 'dark' ? '#1e293b' : '#cbd5e1'} strokeWidth="4" />
                            <circle 
                         ...
                         <span className={`text-xs font-black tracking-tighter ${isActive ? (theme === 'dark' ? 'text-white' : 'text-slate-900') : 'text-slate-600'}`}>{data.streak}</span>
                      </div>
