...
interface ChartsProps {
  ...
  isEditing?: boolean;
  showDisciplinedCurve?: boolean; // Prop propojená s dashboard layoutem
...
const CustomEquityTooltip = ({ active, payload, label, theme }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className={`border p-4 rounded-2xl shadow-2xl z-[1000] backdrop-blur-xl animate-in fade-in zoom-in-95 duration-200 ${
        theme === 'dark' ? 'bg-[#0f172a]/95 border-slate-700 text-white' : 'bg-white/95 border-slate-200 text-slate-900'
      }`}>
        ...
        <div className="space-y-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
               ...
               <span className="text-[10px] font-black uppercase tracking-tight text-slate-400">Reálná Equity</span>
            </div>
          ...
          {validVal !== undefined && (
            <div className="pt-2 border-t border-slate-800">
              <div className="flex items-center gap-2 mb-1">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-tight text-slate-400 text-amber-500/80">Disciplinovaná</span>
              </div>
              <p className={`font-black text-xl leading-none text-amber-400`}>
                ${validVal.toLocaleString()}
              </p>
              <p className="text-[9px] font-bold text-slate-500 mt-1 uppercase">
                Rozdíl: ${Math.abs(validVal - val).toLocaleString()} 
                {validVal > val ? ' (ušlý zisk)' : ' (ušetřeno)'}
              </p>
            </div>
          )}
        </div>
...
const CustomBarTooltip = ({ active, payload, label, theme }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className={`border p-4 rounded-lg shadow-xl z-[1000] min-w-[200px] ${
        theme === 'dark' ? 'bg-[#0f172a] border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'
      }`}>
        ...
        <div className="space-y-1.5 text-sm">
          <div className="flex justify-between items-center">
             ...
             <span className="font-mono font-bold text-emerald-400">+${profit.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center">
             <span className="text-rose-400">Hrubá ztráta:</span>
             <span className="font-mono font-bold text-rose-400">-${loss.toLocaleString()}</span>
          </div>
          <div className={`flex justify-between items-center pt-2 mt-1 border-t ${theme === 'dark' ? 'border-slate-800' : 'border-slate-100'}`}>
...
const Charts: React.FC<ChartsProps> = ({ stats, theme, onlyEquity, onlyDistribution, isEditing, showDisciplinedCurve, onToggleDisciplined }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* 1. EQUITY CURVE */}
      {!onlyDistribution && (
        <div className={`p-6 rounded-[32px] border transition-all overflow-visible ${
          theme === 'dark' ? 'bg-[#0a0f1d]/90 border-white/5 backdrop-blur-xl' : 'bg-white border-slate-200 shadow-sm'
        }`}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
               <h3 className={`text-xs font-black uppercase tracking-widest flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            ...
            <div className="flex items-center gap-3">
               ...
               {isEditing && (
                 <button 
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[10px] font-black uppercase transition-all active:scale-95 animate-in slide-in-from-right-4 duration-300 ${
               ...
               <div className="flex flex-col items-end">
                  ...
                  <span className="text-sm font-mono font-black text-blue-500">${initialCap.toLocaleString()}</span>
               </div>
            </div>
          </div>
            ...
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={hourData} stackOffset="sign" margin={{ top: 20, right: 30, left: 20, bottom: 5 }} barGap={0}>
                  ...
                  <Tooltip content={<CustomBarTooltip theme={theme} />} cursor={{ fill: theme === 'dark' ? '#334155' : '#e2e8f0', opacity: 0.1 }} allowEscapeViewBox={{ x: true, y: true }} />
