...
import { loadMarketCandles, resolveMarketSymbol, type MarketCandle } from '../services/marketData';
import {
  cancelBacktestOrder,
  createBacktestOrder,
...
const SEGMENT_MS = 14 * 24 * 60 * 60 * 1_000;
...
const toTrade = (closed: BacktestClosedTrade, accountId: string): Trade => ({
  id: closed.id,
  accountId,
  ...
  duration: `${Math.max(0, Math.round((closed.exitTime - closed.entryTime) / 60))}m`,
  durationMinutes: Math.max(0, Math.round((closed.exitTime - closed.entryTime) / 60)),
...
const BacktestWorkspace: React.FC<Props> = ({ run: initialRun, isDark, onClose, onTradeClosed }) => {
  ...
  const emitClosedTrades = useCallback((runtime: BacktestRuntimeState) => {
    runtime.closedTrades.forEach(closed => {
      if (emittedTradesRef.current.has(closed.id)) return;
      emittedTradesRef.current.add(closed.id);
      onTradeClosed?.(toTrade(closed, runRef.current.accountId));
    });
  }, [onTradeClosed]);

  const handleReplayChange = useCallback((replay: ChartReplayState) => {
    const executionCandles = candlesByRoot.MNQ ?? [];
    ...
    if (replay.phase === 'active' && replay.cursorTime !== null && (previous === null || replay.cursorTime > previous)) {
      ...
      revealed.forEach(candle => { runtime = processBacktestCandle(runtime, runRef.current.id, 'MNQ', candle, runRef.current.config); });
  ...
  }, [candlesByRoot.MNQ, emitClosedTrades, loadSegment, updateRun]);
  ...
  const executeOrder = useCallback((input: {
    ...
    updateRun(current => {
      let runtime = enqueueBacktestOrder(current.runtimeState, order);
  ...
  const cancelOrder = useCallback((id: string, candle: MarketCandle | null) => {
    updateRun(current => ({ ...current, runtimeState: cancelBacktestOrder(current.runtimeState, id, candle?.time ?? Math.floor(Date.now() / 1_000)), updatedAt: Date.now() }));
  }, [updateRun]);

  const changeBracket = useCallback((stopLoss?: number, takeProfit?: number) => {
    updateRun(current => ({
      ...current,
      runtimeState: updatePositionBracket(current.runtimeState, current.executionSymbol, stopLoss, takeProfit),
      updatedAt: Date.now(),
    }));
  }, [updateRun]);
  ...
  }), [candlesByRoot, changeBracket, closePosition, executeOrder, handleReplayChange, handleWorkspaceChange, isDark, run.endAt, run.id, run.runtimeState, run.startAt, run.workspaceState]);
  ...
  const syntheticTrade = useMemo<Trade>(() => ({
    ...
    signal: 'Bar Replay',
    pnl: run.runtimeState.realizedPnl,
    runUp: 0,
    drawdown: 0,
    ...
    duration: '',
    durationMinutes: 0,
  }), [run.accountId, run.executionSymbol, run.id, run.runtimeState.realizedPnl, run.startAt]);
...
const BacktestTradingPanel: React.FC<TradingPanelProps> = ({ run, candle, isDark, onOrder, onCancel, onClosePosition, onChangeBracket }) => {
  const [type, setType] = useState<BacktestOrderType>('market');
  const [quantity, setQuantity] = useState(run.config.defaultQuantity || 1);
    ...
    <div className="flex h-full items-center gap-3 overflow-x-auto px-3 text-[10px]">
      <div className="min-w-[245px]">
        <div className="mb-1 flex gap-1">
          ...
          <input type="number" min={1} value={quantity} onChange={event => setQuantity(Math.max(1, Number(event.target.value)))} className={`${field} w-14`} title="Počet kontraktů" />
