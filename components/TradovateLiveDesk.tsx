...
import { LIVE_READ_MAX_AGE_MS, LIVE_READ_STALE_MS, formatReadAge } from '../lib/liveReadFreshness';
    ...
    <div className="mx-auto max-w-[1500px] space-y-4">
      ...
      {accountsWithoutPlan.length > 0 && (
        <div className="flex flex-col gap-3 rounded-lg border border-amber-500/35 bg-amber-500/10 p-4 text-amber-700 sm:flex-row sm:items-center dark:text-amber-400">
          ...
          <button type="button" onClick={() => { setProfileSetupAccountIds(new Set(accountsWithoutPlan.map(account => String(account.id)))); live.setProfileSetupOpen(true); }} className="h-9 shrink-0 rounded-md bg-amber-500 px-4 text-xs font-black text-slate-950">
            Přiřadit plán
          </button>
        </div>
      )}
      ...
      {tab === 'connections' ? (
        <>
          <Connections
            ...
            onProfiles={connectionId => {
              setProfileSetupAccountIds(new Set((live.connectionData[connectionId]?.accounts ?? []).map(account => String(account.id))));
      ...
      {live.profileSetupOpen && liveData ? (
        <TradovateAccountProfileSetup
          ...
          onSaved={profiles => {
            ...
            setProfileSetupAccountIds(null);
            live.setError(null);
          }}
        />
      ) : null}
      ...
      <section className="overflow-hidden rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)]">
        ...
        {connections.length === 0 && busy !== 'status' ? <EmptyConnection onAdd={onAdd} /> : connections.length > 0 ? (
          connections.map(connection => {
            ...
            const profilesByAccount = new Map(profiles.map(profile => [profile.externalAccountId, profile]));
            const propFirms = Array.from(new Set((dataset?.accounts ?? []).map(account => tradovateAccountFirm(profilesByAccount.get(String(account.id)), account.name)).filter(Boolean)));
...
const PositionsAndOrders = ({ data, profiles }: { data: TradovatePreflightResult; profiles: TradovateAccountProfile[] }) => {
  ...
  // Poslední známé hodnoty zůstávají; stáří se ukáže jen jako podtitul.
  const oldestReadAge = (source: 'positions' | 'orders') => {
    const ages = data.accounts.map(account => {
      const state = tradovateAccountReadState(account, data);
      const asOf = Date.parse(state[source === 'positions' ? 'positionsAsOf' : 'ordersAsOf'] ?? '');
      return Number.isFinite(asOf) ? Math.max(0, Date.now() - asOf) : null;
    });
    return ages.some(age => age == null) ? null : Math.max(0, ...ages.map(age => age ?? 0));
  };
  const staleSub = (source: 'positions' | 'orders', fresh: string) => {
    const age = oldestReadAge(source);
    return age == null ? 'Poslední známé · čtení nedostupné' : age > LIVE_READ_STALE_MS ? `Poslední známé · před ${formatReadAge(age)}` : fresh;
  };
