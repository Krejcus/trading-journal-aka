...
import { buildTradeGroupIndex, isCombinedTrade, tradeAccountLabel, tradeGroupMembers } from '../lib/tradeHistoryPresentation';
...
export default function LiveJournalHistory({ trades, accounts, mode, onMode, onSelect, onHistory }: {
  ...
  return <section aria-label="Historie kopírovaných obchodů" className="overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] text-[var(--text-primary)]">
    ...
    {open && <>
      <p className="px-5 pb-3 text-[11px] text-[var(--text-secondary)]">Uzavřené obchody s doloženým výsledkem · stejné filtry účtů a období jako v historii</p>
      {visible.length ? <div className="divide-y divide-[var(--border-subtle)] border-t border-[var(--border-subtle)]">
        {visible.map(trade => {
          ...
          return <button type="button" key={trade.id} onClick={() => onSelect(trade)} className="flex w-full flex-wrap items-center gap-x-5 gap-y-2 px-5 py-3 text-left hover:bg-indigo-500/[0.035] focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-500">
            ...
            <span className="min-w-0 flex-1 truncate text-xs text-[var(--text-secondary)]">{combined ? `${tradeAccountLabel(members)} s plněním` : names.get(trade.accountId) ?? trade.accountId}</span>
      ...
      </div> : <p className="border-t border-[var(--border-subtle)] px-5 py-6 text-xs text-[var(--text-secondary)]">Pro vybrané filtry zatím není dostupný potvrzený uzavřený obchod.</p>}
