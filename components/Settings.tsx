...
const EmojiPicker = ({ onSelect, onClose, isDark }: { onSelect: (e: string) => void, onClose: () => void, isDark: boolean }) => (
  <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
    <motion.div
      ...
      className={`p-6 rounded-[32px] border shadow-2xl max-w-[280px] ${isDark ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-200'}`}
      ...
      <div className="grid grid-cols-5 gap-3">
        {COMMON_EMOJIS.map(emoji => (
          <button
            ...
            className={`w-10 h-10 rounded-xl text-xl flex items-center justify-center transition-all ${isDark ? 'hover:bg-white/10 active:bg-white/20' : 'hover:bg-slate-100 active:bg-slate-200'}`}
          >
            {emoji}
          </button>
        ))}
