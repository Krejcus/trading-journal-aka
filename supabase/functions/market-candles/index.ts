...
Deno.serve(async (req: Request) => {
  ...
  let payload: { symbol?: unknown; start?: unknown; end?: unknown };
  try {
  ...
  if (!validateRequestedRange(requestedStart.getTime(), requestedEnd.getTime())) {
    return json({ error: 'invalid-range', message: 'Okno musí být kladné a nejvýše 16 dní.' }, 400);
  }
  ...
  return json({
    provider: 'databento', dataset: DATASET, symbol, sourceSymbol: parsed.sourceSymbol,
