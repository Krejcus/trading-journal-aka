...
const App: React.FC = () => {
    ...
    if (needsBusinessData && session && !isBusinessDataLoaded) {
      ...
      Promise.all([
        ...
        const stableSet = <T,>(setter: React.Dispatch<React.SetStateAction<T[]>>, next: T[]) => {
          setter(prev => !isCurrentSession() ? prev : businessDataFingerprint(next) === businessDataFingerprint(prev) ? prev : next);
        };
        ...
        if (payouts && payouts.length > 0) {
          storageService.prefetchPayoutImages().then(imageMap => {
            if (!isCurrentSession()) return;
            if (imageMap.size > 0) {
              setBusinessPayouts(prev => !isCurrentSession() ? prev : prev.map(p => {
  ...
  }, [activePage, session, isBusinessDataLoaded, captureSessionRequest]);
