...
Version 0.2.0 (build 2) is currently a **local, uninstalled candidate**. Its
server routes, PWA device management, and isolated Supabase migration have not
been deployed or applied. The installed app in
`/Users/filipkrejca/Applications/AlphaTrade Status.app` remains the mock-only
0.1 build, and its existing LaunchAgent still supplies the `live` fixture.
Do not replace either until the production backup, migration, deploy, real E2E
pairing/revocation checks, and a separate user approval are complete.
...
The current local gate is 29/29 XCTest plus a clean Release build. This is not
